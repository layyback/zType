...
ig.baked=true;ig.module('game.menus.stats').requires('game.menus.base','game.xhr').defines(function(){StatsView=ig.Class.extend({data:null,workingIndicator:['','.','..','...'],font:new ig.Font('media/fonts/avenir-36-white.png'),timer:null,width:432,height:216,isExcerpt:false,totalGames:0,init:function(width,height){this.width=width;this.height=height;this.timer=new ig.Timer();},submit:function(data){if(window.Cocoon||window.Ejecta){var games=JSON.parse(localStorage.getItem('stats')||'[]');if(games.length>30){games.shift();}
data.created=(Date.now()/1000)|0;games.push(data);localStorage.setItem('stats',JSON.stringify(games));this.receivedData({games:games});}
...
localStorage.setItem('soundVolume',ig.soundManager.volume);}});MenuItemMusicVolume=MenuItem.extend({wide:true,getText:function(){return(ig.music.volume*100).round()+'%';},left:function(){ig.music.volume=(ig.music.volume-0.1).limit(0,1);},right:function(){ig.music.volume=(ig.music.volume+0.1).limit(0,1);},click:function(){if(ig.input.mouse.x>240){this.right();}else{this.left();}
localStorage.setItem('musicVolume',ig.music.volume);}});MenuItemKeyboardLayout=MenuItem.extend({wide:true,init:function(){this.layouts=[];this.current=0;var selected=localStorage.getItem('keyboardLayout');var i=0;for(var k in ig.Keyboard.LAYOUT){var layout=ig.Keyboard.LAYOUT[k];this.layouts.push({name:layout.name,layout:layout,key:k});if(selected===k){this.current=this.layouts.length-1;}}},getText:function(){return this.layouts[this.current].name;},left:function(){this.current--;if(this.current<0){this.current=this.layouts.length-1;}
...
localStorage.setItem('keyboardLayout',this.layouts[this.current].key);}});MenuItemResume=MenuItem.extend({getText:function(){return'resume';},ok:function(){ig.game.menu=null;}});MenuSettings=Menu.extend({scale:0.8,clearColor:'rgba(0,0,0,0.8)',init:function(){if(ig.game.mode!==ZType.MODE.GAME){this.itemClasses.erase(MenuItemResume);}
...
if(!ig.game.targets[w.charAt(0).toLowerCase()].length){return w;}}
...
var numSparks=numParticles/2;for(var i=0;i<numSparks;i++){ig.game.spawnEntity(EntityExplosionSpark,this.pos.x,this.pos.y);}},isHitBy:function(letter){var expected=ig.game.translateUmlaut(this.remainingWord.charAt(0).toLowerCase());if(expected==letter.toLowerCase()){this.remainingWord=this.remainingWord.substr(1);if(this.remainingWord.length==0){ig.game.currentTarget=null;this.dead=true;}
return true;}
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.menus.about','game.menus.game-over','game.menus.settings','game.menus.title','game.entities.enemy-missle','game.entities.enemy-mine','game.entities.enemy-destroyer','game.entities.enemy-oppressor','game.entities.player','game.keyboard','game.xhr','game.ease','plugins.silent-loader','plugins.rise-loader','game.document-scanner','game.words.en').defines(function(){Number.zeroes='000000000000';Number.prototype.zeroFill=function(d){var s=this.toString();return Number.zeroes.substr(0,d-s.length)+s;};ZType=ig.Game.extend({font:new ig.Font('media/fonts/avenir-18-white.png'),fontTitle:new ig.Font('media/fonts/avenir-36-blue.png'),separatorBar:new ig.Image('media/ui/bar-blue.png'),idleTimer:null,spawnTimer:null,targets:{},currentTarget:null,yScroll:0,yScroll2:0,gradient:new ig.Image('media/background/gradient.png'),stars:new ig.Image('media/background/stars.jpg'),grid:new ig.Image('media/background/grid.png'),music1:new ig.Sound('media/music/endure.ogg',false),music2:new ig.Sound('media/music/orientation.ogg',false),cancelSound:new ig.Sound('media/sounds/cancel.ogg'),spawnSound:new ig.Sound('media/sounds/spawn.ogg'),menu:null,mode:0,score:0,streak:0,hits:0,misses:0,multiplier:1,wave:{},gameTime:0,kills:0,emps:0,personalBest:0,isPersonalBest:false,waitingForItunes:false,adPage:null,difficulty:(ig.ua.mobile?'MOBILE':'DESKTOP'),keyboard:null,_screenShake:0,wordlist:null,init:function(){if(ig.doc&&ig.doc.fragments.length<2){ig.doc=null;}
this.fontTitle.letterSpacing=-2;this.font.letterSpacing=-1;var bgmap=new ig.BackgroundMap(620,[[1]],this.grid);bgmap.repeat=true;this.backgroundMaps.push(bgmap);ig.music.add(this.music1);ig.music.add(this.music2);ig.music.loop=true;ig.music.random=true;var soundVolume=localStorage.getItem('soundVolume');var musicVolume=localStorage.getItem('musicVolume');if(soundVolume!==null&&musicVolume!==null){ig.soundManager.volume=parseFloat(soundVolume);ig.music.volume=parseFloat(musicVolume);}
else{ig.soundManager.volume=0.5;ig.music.volume=0.5;}
window.addEventListener('keypress',this.keypress.bind(this),false);window.addEventListener('keydown',this.keydown.bind(this),false);var layoutKey=localStorage.getItem('keyboardLayout');var layout=layoutKey?ig.Keyboard.LAYOUT[layoutKey]:null;this.keyboard=new ig.Keyboard(this.virtualKeydown.bind(this),layout);ig.input.bind(ig.KEY.ENTER,'ok');ig.input.bind(ig.KEY.SPACE,'ok');ig.input.bind(ig.KEY.MOUSE1,'click');ig.input.bind(ig.KEY.ESC,'menu');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.system.canvas.onclick=function(){window.focus();};this.personalBest=parseInt(localStorage.getItem('highscore'))|0;if(window.Ejecta){this.gameCenter=new Ejecta.GameCenter();this.gameCenter.authenticate();if(!localStorage.getItem('removeAds')){this.adPage=new Ejecta.AdMobPage("ca-app-pub-8533552145182353/1344920700");}}
...
this.setTitle();this.wordlist=ig.WORDS.EN;if(ig.doc){this.reAllWordCharacter=/^[a-zßàáâãäåæçèéêëìíîïðñòóôõöøùúûüý]+$/i;this.reSplitNonWord=/[^0-9a-zßàáâãäåæçèéêëìíîïðñòóôõöøùúûüý]/i;ig.doc.fastForwardScanAnimation();}},reset:function(){this.entities=[];this.currentTarget=null;this.wave=ig.copy(ZType.WAVES[this.difficulty]);var first='a'.charCodeAt(0),last='z'.charCodeAt(0);for(var i=first;i<=last;i++){this.targets[String.fromCharCode(i)]=[];}
for(var c in this._umlautTable){this.targets[c]=[];}
this.score=0;this.rs=0;this.streak=0;this.longestStreak=0;this.hits=0;this.misses=0;this.kills=0;this.multiplier=1;this.gameTime=0;this.isPersonalBest=false;this.speedFactor=1;this.lastKillTimer=new ig.Timer();this.spawnTimer=new ig.Timer();this.idleTimer=new ig.Timer();this.waveEndTimer=null;},nextWave:function(){this.wave.wave++;this.wave.spawnWait=(this.wave.spawnWait*0.97).limit(0.2,1);this.wave.currentSpawnWait=this.wave.spawnWait;this.wave.spawn=[];this.speedFactor*=this.wave.speedIncrease;if(ig.doc){for(var i=0;i<10&&this.wave.spawn.length<2;i++){this.nextDocFragment();}
...
var words=fragment.text.replace(/['’‘’’]/g,'').split(this.reSplitNonWord);var filteredWords=[];for(var i=0;i<words.length;i++){var w=words[i].trim();if(w.match(this.reAllWordCharacter)){filteredWords.push(w);}}
...
return ent;},registerTarget:function(letter,ent){var c=this.translateUmlaut(letter.toLowerCase());this.targets[c].push(ent);if(!this.currentTarget){this.setExpectedKeys();}},unregisterTarget:function(letter,ent){var c=this.translateUmlaut(letter.toLowerCase());this.targets[c].erase(ent);if(!this.currentTarget){this.setExpectedKeys();}},setExpectedKeys:function(){this.keyboard.expectedKeys=[];for(var k in this.targets){if(this.targets[k].length){this.keyboard.expectedKeys.push(k);}}},_umlautTable:{'ß':'s','à':'a','á':'a','â':'a','ã':'a','ä':'a','å':'a','æ':'a','ç':'c','è':'e','é':'e','ê':'e','ë':'e','ì':'i','í':'i','î':'i','ï':'i','ð':'d','ñ':'n','ò':'o','ó':'o','ô':'o','õ':'o','ö':'o','ø':'o','ù':'u','ú':'u','û':'u','ü':'u','ý':'y'},translateUmlaut:function(k){if(ig.ua.mobile||(ig.doc&&ig.doc.looksLikeEnglish)){return this._umlautTable[k]||k;}
else{return k;}},keypress:function(ev){if(ev.target.tagName=='INPUT'||ev.ctrlKey||ev.altKey||this.mode!=ZType.MODE.GAME||this.menu){return true;}
var c=ev.charCode;if(c<64){return true;}
ev.stopPropagation();ev.preventDefault();var letter=String.fromCharCode(c).toLowerCase();this.shoot(letter);return false;},keydown:function(ev){if(ev.target.tagName=='INPUT'||ev.ctrlKey||ev.altKey||this.mode!=ZType.MODE.GAME||this.menu){return true;}
...
return true;}
this.shoot(letter);},shoot:function(letter){this.idleTimer.reset();if(!this.currentTarget){var potentialTargets=this.targets[letter];var nearestDistance=-1;var nearestTarget=null;for(var i=0;i<potentialTargets.length;i++){var distance=this.player.distanceTo(potentialTargets[i]);if(distance<nearestDistance||!nearestTarget){nearestDistance=distance;nearestTarget=potentialTargets[i];}}
if(nearestTarget){nearestTarget.target();}
else{this.player.miss();this.multiplier=1;this.streak=0;this.misses++;}}
if(this.currentTarget){var target=this.currentTarget;var hit=this.currentTarget.isHitBy(letter);if(hit){this.player.shoot(target);this.score+=this.multiplier;this.hits++;this.streak++;this.longestStreak=Math.max(this.streak,this.longestStreak);if(ZType.MULTIPLIER_TIERS[this.streak]){this.multiplier=ZType.MULTIPLIER_TIERS[this.streak];this.keyboard.showMultiplier(this.multiplier);}
if(target.dead){this.kills++;this.setExpectedKeys();}
else{var translated=this.translateUmlaut(target.remainingWord.charAt(0).toLowerCase());if(this.keyboard){this.keyboard.expectedKeys=[translated];}}}
else{this.player.miss();this.multiplier=1;this.streak=0;this.misses++;}}},setGame:function(){this.setGameState();this.gameTransitionTimer=new ig.Timer(2);this.nextWave();},setGameState:function(){this.reset();var sx=ig.system.width/2-6,sy=ig.system.height-this.keyboard.height*this.keyboard.drawScale-30;this.player=this.spawnEntity(EntityPlayer,sx,sy);this.mode=ZType.MODE.GAME;ig.music.next();this.spawnSound.play();this.emps=3;},continue:function(){var fragment=this.wave.fragment;this.setGameState();this.wave.fragment=Math.max(0,fragment-1);this.nextWave();this.menu=null;},setGameOver:function(){if(this.score>this.personalBest){this.isPersonalBest=true;this.personalBest=this.score;localStorage.setItem('highscore',this.personalBest);}
if(this.gameCenter&&this.score>5){this.gameCenter.reportScore('score',this.score);}
this.mode=ZType.MODE.GAME_OVER;ig.music.fadeOut(1);},showGameOverScreen:function(){this.menu=new MenuGameOver();if(this.adPage&&!localStorage.getItem('removeAds')){this.adPage.show();}
...
this.parent();if(this.mode===ZType.MODE.GAME){this.spawnCurrentWave();if(!this.menu&&!ig.ua.mobile&&ig.input.pressed('click')&&ig.input.mouse.x<64&&ig.input.mouse.y<64){this.menu=new MenuSettings();}}
...
else{localStorage.setItem('removeAds',true);}
...
else{for(var i=0;i<transactions.length;i++){if(transactions[i].productId=='removeAds'){localStorage.setItem('removeAds',true);ig.game.setTitle();return;}}}
ig.game.setTitle();});}});ZType.MODE={TITLE:0,GAME:1,GAME_OVER:2};ZType.MULTIPLIER_TIERS={20:2,50:3};ZType.WAVES={MOBILE:{fragment:0,wave:0,spawn:[],spawnWait:1,healthBoost:0,speedIncrease:1.01,types:[{type:EntityEnemyOppressor,count:0,incEvery:9},{type:EntityEnemyDestroyer,count:0,incEvery:4},{type:EntityEnemyMine,count:3,incEvery:1}]},DESKTOP:{fragment:0,wave:0,spawn:[],spawnWait:0.7,healthBoost:0,speedIncrease:1.05,types:[{type:EntityEnemyOppressor,count:0,incEvery:7},{type:EntityEnemyDestroyer,count:0,incEvery:3},{type:EntityEnemyMine,count:3,incEvery:1}]}};var canvas=ig.$('#ztype-game-canvas')||ig.$('#canvas');var width=480;var height=720;if(ig.ua.mobile){if(ig.$('#ztype-gsense')&&ig.$('#ztype-byline')){ig.$('#ztype-gsense').style.display='none';ig.$('#ztype-byline').style.display='none';}
...
if(window.ZTypeDocumentMode){ig.doc=new ig.DocumentScanner(document.body);if(!ig.ua.mobile){ig.doc.playScanAnimation(function(){if(!ig.ua.mobile){(adsbygoogle=window.adsbygoogle||[]).push({});}});}}
